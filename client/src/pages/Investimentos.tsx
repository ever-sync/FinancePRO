...
export default function Investimentos() {
  ...
  const totalDeposited = Number(items?.summary.totalDeposited ?? 0);
  const totalBalance = Number(items?.summary.totalBalance ?? 0);
  const totalYield = Number(items?.summary.totalYield ?? 0);
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Controle dos seus investimentos pessoais (10% do pró-labore)
          </p>
        </div>
