...
export default function ContasFixas() {
  ...
  const { data: items, isLoading } = trpc.personalFixedCosts.list.useQuery({
    month,
    year,
    page,
    limit,
  ...
  const createMut = trpc.personalFixedCosts.create.useMutation({
    onSuccess: () => {
      utils.personalFixedCosts.list.invalidate();
      toast.success("Conta adicionada");
      setOpen(false);
    },
  });
  const updateMut = trpc.personalFixedCosts.update.useMutation({
    onSuccess: () => {
      utils.personalFixedCosts.list.invalidate();
    },
  });
  const deleteMut = trpc.personalFixedCosts.delete.useMutation({
    onSuccess: () => {
      utils.personalFixedCosts.list.invalidate();
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">
            Contas Fixas - Pessoal
          </h1>
          <p className="text-sm text-muted-foreground">
            Despesas recorrentes da sua vida pessoal
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Nova Conta Fixa Pessoal</DialogTitle>
              </DialogHeader>
